...
const page = async ({ params }: BlogContentProps) => {
  ...
  return (
    <main className="w-full pt-10 min-h-[calc(100vh-64px)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        {blog.user && blog.user.name && blog.user.image && blog.createdAt && (
          <PostMeta
            ...
            readTime={blog.readtime || "0 min read"}
            date={blog.createdAt}
          />
        )}
        <Reactions
      ...
      <div
        className={`w-full bg-white ${relatedBlogs && relatedBlogs.length > 0 ? "border-t py-10" : "border-t-0 py-0"}`}
      >
        <div
          className={`flex flex-col max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full`}
        >
          <div className="flex flex-col">
            {relatedBlogs && relatedBlogs.length > 0 && (
              <>
                <SectionHeader title="More on this topic" />
                <div className="grid pt-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-10">
