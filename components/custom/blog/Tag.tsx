...
interface TagInputProps {
  ...
  availableTags?: string[]; // made optional
}
...
export default function TagInput({
  ...
  return (
    <div className="w-full relative">
      ...
      <div
        className={`w-full flex flex-wrap items-center border-b py-1 px-2 transition-colors ${
          errors.tags ? 'border-b-rose-400' : 'border-b-border'
        } focus-within:border-b-emerald-500`}
      >
        {(tags ?? []).map((tag, index) => (
          <span
            key={index}
        ...
        {tags.length < maxTags && (
          <div className="relative flex-1 min-w-[100px]">
            <Input
              ...
              placeholder="Add tag"
              className="border-none shadow-none rounded-none outline-none ring-0 focus-visible:ring-0 px-0 py-0"
            />

            {showSuggestions && filteredSuggestions.length > 0 && (
              <ul className="absolute -left-2 -right-2 bg-white mt-2 z-10 border border-gray-200 rounded-md shadow-sm max-h-48 overflow-auto">
                {filteredSuggestions.map((tag) => (
                  <li
                    key={tag}
                    onClick={() => addTag(tag)}
                    className="px-2 py-1 cursor-pointer hover:bg-gray-100 transition-all duration-300"
                  >
                    {tag}
                  </li>
                ))}
